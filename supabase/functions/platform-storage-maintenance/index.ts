...
function env(...names: string[]) {
  ...
  throw new Error(`Missing ${names.join("/")}`);
}
...
Deno.serve(async (req) => {
  ...
  if (req.headers.get("x-cron-secret") !== env("RCE_CRON_SECRET", "NIS_CRON_SECRET")) return json({ ok: false, error: "unauthorised" }, 401);
  const db = createClient(env("SUPABASE_URL"), env("SUPABASE_SERVICE_ROLE_KEY"), { auth: { persistSession: false, autoRefreshToken: false } });
  const cutoff = new Date(Date.now() - RETENTION_DAYS * 86400000).toISOString();
  const result = { finalized_interrupted: 0, packages_pruned: 0, package_failures: 0, template_objects_pruned: 0 };
  try {
    ...
    for (const row of interrupted.data ?? []) {
      ...
      const fin = await db.from("platform_package_artifacts").update({ status: "deleted", deletion_state: "completed", deleted_at: now, deletion_reason: "r35 superseded package retention cleanup", storage_path: `deleted/${row.id}` }).eq("id", row.id).eq("deletion_state", "storage_removed");
    ...
    const candidates = await db.from("platform_package_artifacts").select("id,tenant_code,storage_path,superseded_by_artifact_id,revoked_at,deletion_state").eq("status", "revoked").in("deletion_state", ["none", "failed"]).not("superseded_by_artifact_id", "is", null).lt("revoked_at", cutoff).order("revoked_at", { ascending: true }).limit(50);
    ...
    for (const artifact of candidates.data ?? []) {
      ...
      const now = new Date().toISOString();
      const fin = await db.from("platform_package_artifacts").update({ status: "deleted", deletion_state: "completed", deleted_at: now, deletion_reason: "r35 superseded package retention cleanup", storage_path: `deleted/${artifact.id}` }).eq("id", artifact.id).eq("deletion_state", "storage_removed");
      if (fin.error) throw fin.error;
      ...
      await db.from("platform_package_events").insert({ event_type: "package_deleted", artifact_id: artifact.id, event_reason: "Superseded generated package pruned after retention window", event_data: { release: "r35", previous_storage_path: artifact.storage_path, replacement_artifact_id: artifact.superseded_by_artifact_id, policy: "superseded-package-retention" } });
    ...
    const inactive = (templates.data ?? []).filter((t) => !t.active);
    ...
    for (const t of inactive) {
      ...
      if (new Date(t.updated_at).getTime() >= new Date(cutoff).getTime()) continue;
      const rm = await db.storage.from(TEMPLATE_BUCKET).remove([t.storage_path]);
      if (rm.error) throw rm.error;
      await db.from("platform_package_events").insert({ event_type: "template_storage_pruned", template_id: t.id, event_reason: "Inactive template archive pruned; immediate rollback template retained", event_data: { release: "r35", storage_path: t.storage_path, policy: "one-template-rollback-retention" } });
      result.template_objects_pruned++;
    }
    return json({ ok: true, release: "r35-final-product-stability", retention_days: RETENTION_DAYS, ...result });
  } catch (error) {
    return json({ ok: false, error: error instanceof Error ? error.message : String(error), ...result }, 500);
  }
});
