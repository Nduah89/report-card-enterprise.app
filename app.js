(() => {
  ...
  const ROLE_NAV_IDS = Object.freeze({
    ...
    system_admin:["dashboard","operations","students","history","teachers","headteachers","academics","timetable","delegations","reports","certificates","id_cards","insights","users","notifications","compliance","audit","backup_restore","license_capacity","settings"],
  ...
  const state = {
    ...
    timetableYear:"", timetableClass:"", timetableConsole:null, teacherProfile:null
  };
  ...
  async function init() {
    ...
    state.client.auth.onAuthStateChange(async(event,newSession)=>{
      ...
      if(event==="SIGNED_OUT"){stopGeneratedLicenseRefresh();state.initialized=false;disconnectRealtime();showOnly("authView");}
  ...
  async function navigate(view,force=false) {
    ...
    try {
      const renderer={
        dashboard:renderDashboard,operations:renderOperations,licensing:renderLicensing,teacher_profile:renderTeacherProfile,my_class:renderMyClass,attendance:renderAttendance,my_subjects:renderMySubjects,students:renderStudents,history:renderAcademicHistory,teachers:renderTeachers,headteachers:renderPrincipals,academics:renderAcademics,timetable:renderTimetable,delegations:renderEmergencyDelegations,reports:renderReports,certificates:renderCertificates,id_cards:renderIdCards,insights:renderInsights,
  ...
  function handleRealtime(topic,payload) {
    ...
    if(["profiles","user_class_access","teachers","headteachers","classes","subjects","class_subjects","grading_scales","students","enrollments","student_reports","subject_results","class_attendance_registers","student_attendance_entries","emergency_academic_delegations","academic_period_controls","report_correction_requests","student_lifecycle_events","transcript_issuances","privacy_requests","security_events","recovery_test_runs","certificate_templates","teacher_award_categories","certificate_batches","certificates","certificate_events","id_card_settings","student_id_cards","id_card_events","staff_id_cards","staff_id_card_events","id_card_deletion_tombstones","class_timetable_entries"].includes(table))state.workspace=null;
  ...
  async function openTeacherEditor(id=null) {
    ...
    modal(id?"Edit Teacher":"Add Teacher",row.staff_no||"",`<form id="teacherForm" class="form-stack"><input type="hidden" name="id" value="${attr(row.id||"")}"><input type="hidden" name="updated_at" value="${attr(row.updated_at||"")}"><div class="staff-photo-editor">${photo?`<img src="${attr(photo)}" alt="Teacher photograph">`:`<span>${esc(idCardInitials(row.full_name||fullName(row)||"Teacher"))}</span>`}<label class="field"><span>Teacher photograph</span><input id="teacherPhotoFile" type="file" accept="image/jpeg,image/png,image/webp"><small>Optional. The teacher can also upload or replace this from My Profile.</small></label></div>
      <div class="form-grid three"><label class="field"><span>Staff number</span><input name="staff_no" value="${attr(row.staff_no||"")}" required></label><label class="field"><span>EMIS code (optional)</span><input name="emis_code" value="${attr(row.emis_code||"")}" maxlength="80"></label><label class="field"><span>Linked user account</span><select name="profile_id">${optionList(profiles,"id","full_name",row.profile_id,"No linked account")}</select></label>
